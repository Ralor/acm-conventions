...
typedef llong dp_val;

dp_val dp[66][66]; // dp[l][r] = cnt on l..r (inclusively)
dp_val rec(int l, int r) {
  if( l == r ) return dp[l][r] = 1;
  if( l == r-1 ) return dp[l][r] = 2 + (s[l] == s[r]);
  if( l > r ) return 0;
  if( dp[l][r] != -1 ) return dp[l][r];
  if( s[l] == s[r] )
    return dp[l][r] = 1 + rec(l+1,r) + rec(l,r-1);
  return dp[l][r] = rec(l+1,r) + rec(l,r-1) - rec(l+1,r-1);
}
