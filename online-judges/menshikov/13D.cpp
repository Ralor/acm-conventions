...
int main() {
  ...
  while( cin >> n ) {
    vector<Point> p(n);
    forn(i,n) cin >> p[i].x >> p[i].y, p[i].x += 20*1000 + 1, p[i].y += 20*1000 + 1;

    Point z(0,0);
    double total_area = 0;
    vector<double> area; // signed
    vector<Point> center;
    for(int i = 0, j = 1; i < n; i++, j++, j%=n) {
      center.pb(Point((z.x + p[i].x + p[j].x)/3, (z.y + p[i].y + p[j].y)/3));
      area.pb((p[i] - z) * (p[j] - z) / 2);
      total_area += area[i];
    }

    double c_x = 0, c_y = 0;
    forn(i,sz(area))
      c_x += center[i].x * area[i], c_y += center[i].y * area[i];
    cout << c_x / total_area - 20*1000 - 1 << " " << c_y / total_area - 20*1000 - 1 << "\n";
  }
